...
void printSubject(Subject subject){
    printf("|%-10s|%-20s|%-10s|%-10s|%-10s|%-20s|%-10s|\n", subject.code, subject.course, subject.day, subject.time, subject.period, subject.week, subject.room);
}


void printSchedule_by_day(SchedulePtr root, char* day){
    SchedulePtr cur = root;

    while(cur != NULL){
        if(strcmp(cur->subject.day, day) == 0){
            printSubject(cur->subject);
        }
        cur = cur->nextPtr;
    }
}

char* getSubject_by_day(SchedulePtr root, char* day){
    char *result;
    ...
    while (cur != NULL)
    {
        if (strcmp(cur->subject.day, day) == 0)
        {
            strcat(result, cur->subject.code);
            strcat(result, " ");
            strcat(result, cur->subject.course);
            strcat(result, " ");
            strcat(result, cur->subject.day);
            strcat(result, " ");
            strcat(result, cur->subject.time);
            strcat(result, " ");
            strcat(result, cur->subject.period);
            strcat(result, " ");
            strcat(result, cur->subject.week);
            strcat(result, " ");
            strcat(result, cur->subject.room);
            strcat(result, "\n");
        }
        cur = cur->nextPtr;
    }
